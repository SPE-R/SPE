```{r, include=FALSE}
knitr::opts_chunk$set(results = "hide", prefix.string = "./graph/basic")
```
...
```{r Structure of w,echo=F}
str(w)
```
-  Create the vector `w+1`, and display it.
```{r Create and display w+1,echo=F}
w + 1
```
...
```{r Create a new variable in the workspace }
logbw <- log(births$bweight)
```

produces the variable `logbw` in your workspace,  while

```{r Create a new variable in the data frame  }
...
```{r Joint distribution of sex and hyp,echo=F}
...
```{r Create logical variable early,echo=F }
