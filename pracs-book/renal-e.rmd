...
8.  Now show how the states are connected and the number of transitions
    ...
    ```
    That is presumably not what you want, so
    you may be tempted to read the help page for
    `boxes.Lexis` ...

9.  Draw a `Lexis` diagram where different coloring is
...
10. Make a Cox-regression of mortality rates (i.e. endpoint `ESRD` or
    ...
    ci.exp(m1)
    ```
    There is actually also a wrapper for a Cox model used in a `Lexis`
    object---since the baseline timescale has a special status this is
    givn as the l.h.s. of the formula:
    ```{r }
    ...
    end we again use the function `splitLexis`. The rates can then be
...
11. Now split the follow-up time every month after entry, and verify
    ...
    summary(sLc)
    ```
    What is the difference between the datasets `Lc` and `sLc`?
...
12. Now fit the Poisson-model corresponding to the Cox-model
    ...
    can use the wrapper function `glmLexis`
    ```{r }
    mp <- glmLexis(sLc,
...
13. Try instead using the `gam` function from the
    ...
    ```
    We see that there is virtually no difference between the three
...
16. However, `plot` (well, `plot.gam`) does not give you the *absolute*
    ...
    `glm` model using `Ns`. Any preferences?
...
18. If we want to predict the probability of being in each of the
    ...
    str(inL)
    ```
    The other input for the simulation is the models for the possible transitions.

    This is given as a list with an element for each transient state
    (that is `NRA` and `Rem`), each of which is again a list with
    names equal to the states that can be reached from the transient
    state. The content of the list will be `glm` objects, in this case
    the models we just fitted, describing the transition rates:
...
19. Now generate the life course of, say, 1,000 persons, and look at the summary.
...
23. Now try to improve the plot so that it is easier to read, and
    ...
    ```
    What is the 10-year risk of remission for 50 year old men and
    women respectively?
