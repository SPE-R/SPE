```{r, include=FALSE}
knitr::opts_chunk$set(keep.source = TRUE, include = FALSE, eval = FALSE, prefix.string = "./graph/renal")
```
...
plot(Lr, col = "black", lwd = 3)
```
5. (*Optional, esoteric*) We can produce a slightly more
  ...
  the *total* width of the plot so that the use of `mai`
  (look up the help page for `par`) to specify the margins of
  the plot so that it leaves a plotting area twice as high as wide. The
  `mai` argument to `par` gives the margins in inches,
...
6. Now make a Cox-regression analysis of the enpoint ESRD with
...
7. The main focus of the paper was to assess whether the occurrence of
  ...
  *Remission* is a time-dependent variable which is initially 0, but
  ...
  -  One record for the time before remission, where entry is
    `doe`, exit is `dor`, remission is 0, and event is
    0.
  ...
  state that is *less* severe than *Rem* in the sense that a
  person who see a remission will stay in the *Rem* state unless he
  goes to the *ESRD* state. Also use `split.state=TRUE` to
...
) # which states are less severe
summary(Lc)
```
List the records from a few select persons (choose values for
`lex.id`, using for example `subset`( Lc, lex.id `%in%`
...
plot(Lc,
  col = c("red", "limegreen")[Lc$lex.Cst],
  xlab = "Calendar time", ylab = "Age",
  lwd = 3, grid = 0:20 * 5, xlim = c(1970, 2010), ylim = c(0, 80), xaxs = "i", yaxs = "i", las = 1
)
points(Lc,
  pch = c(NA, NA, 16)[Lc$lex.Xst],
  col = c("red", "limegreen", "transparent")[Lc$lex.Cst]
)
points(Lc,
  pch = c(NA, NA, 1)[Lc$lex.Xst],
  col = "black", lwd = 2
)
```

10. Make Cox-regression of mortality (i.e. endpoint *ESRD* or
...
*rates* be explored. Furthermore, we can allow effects of both
...
12. Try to fit the Poisson-model corresponding to the Cox-model
...
16. However, `plot` does not give you the *absolute*
  ...
  try to predict the rates as a function of `tfi` and the
...
str(nd)
matshade(nd$tfi, cbind(
  ci.pred(mp, newdata = nd),
  ci.pred(mx, newdata = nd)
) * 100,
plot = TRUE,
...
21. With the counts of persons in each state at the
  designated time points (in `nStm`), compute the cumulative fraction over the
  states, arranged in order given by `perm`:
```{r pState}
...
22. Try to plot the cumulative probabilities using the `plot`
  method for `pState` objects:
```{r plot-pp,fig=TRUE}
plot(ppf)
```
Is this useful?
