```{r, include=FALSE}
knitr::opts_chunk$set(keep.source=TRUE,eps=FALSE,include=FALSE,eval=FALSE,prefix.string='./graph/DMDK')
```
...
```{r rates5,fig = TRUE, width = 10, eval = FALSE}
Mcr <- gam.Lexis(subset(SL, sex == "M"),
                 ~ s(  A, bs = "cr", k = 10) +
                   s(  P, bs = "cr", k = 10) +
                   s(dur, bs = "cr", k = 5))
...
-  Fit a poisson model with sex as the explanatory varable and
