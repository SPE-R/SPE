...
- One kg difference in body weight corresponds in average to $0.5mmHg$ difference in (systolic) blood pressures.
- Beer-drinking increases blood pressure by $10mmHg$ in average.
>>>>>>> 3617578583b6fd98e2d682055b7b776b244066df
...
```{r beermodels_1, eval=TRUE, solution=TRUE}
library(Epi)
...
```{r beermodels_2b, eval=TRUE, solution=TRUE}
bdat$bp <- 
  110 + 0.5 * bdat$weight + 10 * bdat$beer + rnorm(1000, 0, 10) 
...
```{r bpmodel, eval=TRUE, solution=TRUE}
...
<<<<<<< HEAD
m2bp <- lm(bp ~ beer + sex, data = bdat) 
ci.lin(m3bp) # the correct model for the total effect 
```

*See the file with solutions for correct solutions and explanations!*
=======
...
```{r dagitty2, message = FALSE, fig.show=FALSE,results=FALSE, fig.keep='none'}
g <- dagitty("dag {
    C <- S -> Y -> U -> D
    C -> Z <- Y
    Z -> D
...
```{r dagitty3, warning=FALSE, message=FALSE, fig.show=TRUE, fig.width=4, fig.height=3}
coordinates(g) <- 
  list(
    x = 
      c(
...
```{r dagitty6, eval=TRUE, solution=TRUE}
bg <- dagitty("dag {
  SEX -> BEER -> BP
  SEX -> WEIGHT -> BP
  }")
...
```{r dagitty7, eval=TRUE, solution=TRUE}
...
>>>>>>> 3617578583b6fd98e2d682055b7b776b244066df
```{r dagitty8, eval=TRUE, solution=TRUE}
...
```{r dagitty9, eval=TRUE, solution=TRUE}
bg2 <- dagitty("dag {
  SEX -> BEER -> BP
  SEX -> WEIGHT -> BP
  BEER -> WEIGHT
...
```{r adjustbp, eval=TRUE, solution=TRUE}
...
```{r threedags, fig.width=8, fig.height=3, fig.show=TRUE}
par(mfrow=c(1,3))
g1 <- dagitty("dag {
	U -> Z -> Y 
	U -> X
...
```{r adjust1, eval=TRUE, solution=TRUE}
>>>>>>> 3617578583b6fd98e2d682055b7b776b244066df
...
```{r adjust2, results=TRUE}
=======
*Apparently no effect of X: this DAG "g1" (a) is possible*

```{r adjust2, eval=TRUE, solution=TRUE}
>>>>>>> 3617578583b6fd98e2d682055b7b776b244066df
...
<<<<<<< HEAD
```{r adjust3, results=TRUE}
=======
*A clear effect of X: this DAG "g2" (b) is not likely*

```{r adjust3, eval=TRUE, solution=TRUE}
>>>>>>> 3617578583b6fd98e2d682055b7b776b244066df
...
```{r adjust4, eval=TRUE, solution=TRUE}
...
```{r mrmod1, eval=TRUE, solution=TRUE}
...
```{r mrmod2, eval=TRUE, solution=TRUE}
