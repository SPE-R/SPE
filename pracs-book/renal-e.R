...
m1 <- coxph(Surv(tfi,             # entry time
                 ...
                 lex.Xst %in% EP) # event
            ~ sex + I((doe - dob - 40) / 10) + # fixed covariates
...
## -----------------------------------------------------------------------------
mL <- coxphLexis(Lc, tfi ~ sex + I((doe - dob - 40) / 10) + # fixed covariates
                                 (lex.Cst == "Rem"))        # time-dependent variable
ci.exp(mL)
...
ci.exp(m1, subset = c("Cst", "doe", "sex")) # from Cox model
...
## ----first-sim----------------------------------------------------------------
(iL <- simLexis(Tr, inL, N = 5))
summary(iL, by = "sex")


## ----1000-sim-----------------------------------------------------------------
system.time(sM <- simLexis(Tr, inL, N = 5000, t.range = 12))
summary(sM, by = "sex")
