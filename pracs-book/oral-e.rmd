...
```{r Survobject, echo=TRUE,eval=FALSE}
...
```{r cdfstage, echo=TRUE, fig=TRUE, height=6, width=7,eval=FALSE}
...
```{r chstage, echo=TRUE, fig=TRUE, height=6, width=12,eval=FALSE}
par(mfrow = c(1, 2))
...
```{r plotcif1, echo=TRUE,fig=TRUE,eval=FALSE}
par(mfrow = c(1, 2))
...
```{r plotcif2, echo=TRUE, fig=TRUE, height=6, width=10,eval=FALSE}
col5 <- c("green", "blue", "black", "red", "gray")
...
```{r cox2cll, echo=TRUE,eval=FALSE}
newd <- data.frame(
...
```{r coxhaz1, echo=TRUE,eval=FALSE}
m2haz1 <- 
  coxph(
...
```{r coxhaz2, echo=TRUE,eval=FALSE}
m2haz2 <- 
  coxph(
...
```{r lexis 1, echo=TRUE,eval=FALSE}
orca.lex <- Lexis(
...
```{r split b, echo=TRUE,eval=FALSE}
...
```{r poisson, echo=TRUE,eval=FALSE}
m2pois1 <- glm(
...
for the timebands
```{r poissonresults, echo=TRUE, fig=TRUE,eval=FALSE}
...
```{r poissonspline, echo=TRUE, fig=TRUE,eval=FALSE}
library(splines)
m2pspli <- 
  update(
    m2pois1, 
