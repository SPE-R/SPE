...
## ----beermodels_1, eval=TRUE, solution=TRUE-----------------------------------
library(Epi)
...
## ----beermodels_2b, eval=TRUE, solution=TRUE----------------------------------
bdat$bp <- 
  110 + 0.5 * bdat$weight + 10 * bdat$beer + rnorm(1000, 0, 10) 
...
## ----bpmodel, eval=TRUE, solution=TRUE----------------------------------------
...
## ----dagitty2, message = FALSE, fig.show=FALSE,results=FALSE, fig.keep='none'----
g <- dagitty("dag {
    C <- S -> Y -> U -> D
    C -> Z <- Y
    Z -> D
...
## ----dagitty3, warning=FALSE, message=FALSE, fig.show=TRUE, fig.width=4, fig.height=3----
coordinates(g) <- 
  list(
    x = 
      c(
...
## ----dagitty6, eval=TRUE, solution=TRUE---------------------------------------
bg <- dagitty("dag {
  SEX -> BEER -> BP
  SEX -> WEIGHT -> BP
  }")
...
## ----dagitty7, eval=TRUE, solution=TRUE---------------------------------------
paths(bg, "BEER", "WEIGHT")


## ----dagitty8, eval=TRUE, solution=TRUE---------------------------------------
adjustmentSets(bg, exposure = "BEER", outcome = "WEIGHT")


## ----dagitty9, eval=TRUE, solution=TRUE---------------------------------------
bg2 <- dagitty("dag {
  SEX -> BEER -> BP
  SEX -> WEIGHT -> BP
  BEER -> WEIGHT
...
## ----mrmod1, eval=TRUE, solution=TRUE-----------------------------------------
mxy <- lm(Y ~ BMI, data = mrdat)
ci.lin(mxy)


## ----mrmod2, eval=TRUE, solution=TRUE-----------------------------------------
