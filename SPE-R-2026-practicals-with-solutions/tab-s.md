...
to use depends on your goal.

There are at least three different uses for tables:

- to create simple summary statistics that will be used for further calculations in `R`. For example, a two-by-two table created by the `table` function can be passed to `fisher.test`, which will calculate odds ratios and confidence intervals.  
Similarly, the `group by` and `summarise` functions within the `tidyverse` framework can be used for such purposes (see 3.7). 
The appearance of these tables may, however, be quite basic,
as their principal goal is to create new objects for future calculations;

- to make *publication quality* tables for various reports, presentations and manuscripts.  In addition to the `kableExtra` 
package that was briefly introduced in chapter 3.15 there are several others, like `xtable`, for instance (not covered by this course);

- to create human-readable tables for discussion within your work-group, but not for publication. The
`Epi` package provides a function `stat.table` for this purpose,
and this practical is designed to introduce this function.

## The births data

We shall again use the births data on 500 mothers who had
