...
mel$age.cat <- cut(mel$age, breaks = c(20, 30, 40, 50, 60, 70, 85), right = FALSE)
mel$freckles <- factor(mel$freckles, labels = c("none", "some", "many"))

mel$hair2 <- Relevel(mel$hair, list("dark" = 1, "other" = c(2, 3, 4)))
mel$nvsma4 <- cut(mel$nvsmall, breaks = c(0, 1, 2, 5, 50), right = FALSE)
mel$nvlar3 <- cut(mel$nvlarge, breaks = c(0, 1, 2, 15), right = FALSE)
