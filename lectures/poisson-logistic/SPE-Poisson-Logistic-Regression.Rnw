...
\title{Poisson and Logistic Regression \newline }
...
\end{lstlisting}
Two R-implementations of the rate estimation with Poisson regression:\vspace*{-0.5cm}
\begin{columns}
...
The likelihood \vspace*{0.2in} $L(\lambda)=\prod_{i=1}^{n} \lambda^{\delta_i} e^{-\lambda y_i}$ and $log(L)= \sum\limits_{i=1}^{n} \left[  \delta_i log(\lambda)-\lambda y_i \right]$
...
\begin{center}
<<test, echo=FALSE,eval=TRUE,results=tex>>=
library(kableExtra)
...
\begin{footnotesize}
<<test, echo=TRUE,eval=FALSE,results=tex>>=
library(Epi)
...
\footnotesize{
<<PR, echo=TRUE,eval=TRUE,results=tex,>>=
library(Epi)
library(xtable)
...
\item In Poisson models the response can be either:
  \begin{itemize}
    ...
    \item rate {\tt d/y} with {\tt weights = y} or
    \item case and person-years {\tt c(d,y)} as response in glm with poisreg-family (Epi-package)
  \end{itemize}
\item Both may be fitted on either grouped data, or individual
  records.
\item Binary outcome can be modeled with odds.
