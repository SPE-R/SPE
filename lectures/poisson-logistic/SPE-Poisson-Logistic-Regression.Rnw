...
\begin{frame}[fragile]
\frametitle{Motivation and points to be covered}
Why regression models in the first place ?
\begin{itemize}
\item Want to analyze the time until the event of interest happens.
\item Regression models are used to relate time(s) in which event occurs to \bf{the covariates or factors} of interest
\end{itemize} \vspace{0.2cm}
Points to be covered
\begin{itemize}
\item Incidence rates, rate ratios and rate differences from\\ {\bf follow-up studies}
 can be computed by fitting {\it Poisson regression models}.
\item Risk ratios and differences can be computed from binary data by fitting \\ {\bf Logistic regression models}.
\item Both models are special instances of {\bf Generalized linear models}.
