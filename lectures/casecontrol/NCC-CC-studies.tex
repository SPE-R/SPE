...
Lyon, France, 14 to 20 June, 2018\\
\end{frame}
%\input{#4}
}
...
% \bf Michael Hills\sf, (retired), Highgate, London\\%[1ex]
 \bf Esa Läärä\sf, Unit of Math. Sciences, University of Oulu\\%[1ex]
 \bf Janne Pitkäniemi\sf, Finnish Cancer Registry\\
...
%% Tuesday 29
 \banner{Nested case-control studies and case-cohort studies}%
       {Monday, 18 June, 2018  \\{ } }%
...
{\bf (C) Concurrent sampling} or {\bf density sampling}  
	...
	%			  for the outcome 
         ...
         \pause                  
         \medskip                  
          \item[ ] a.k.a. \textbf{nested case-control design} (NCC)                
...
(without or with time-matching, {\it i.e.} NCC) 
 \pause
\bi
...
\pause
 ...
 \item
  ...
  reported as having estimated an OR ({\it e.g.} Simen Kapeu {\it et al.})
\ei
\pause
...
the empirical {\bf exposure odds ratio} (EOR) 
$$
  \text{EOR} 
...
of controls (unmatched), estimated variance of log(EOR) is \pause
\bes
...
\pause
\begin{itemize}
\item Depends basically on the numbers of cases, when there are  
...
\pause
\bi
% \item Assume first a simple situation: 
% Prevalence of exposure % $q(t)$  in the study population is constant % $q$ % over time.
\pause
\medskip
\item To put it simply: Exposure odds $C_1/C_0$ among controls
 = consistent estimator of exposure odds $Y_1/Y_0$ of person-times.
%% even if controls sampled at any time from population at risk.
\pause
\medskip
\item
Therefore, crude EOR = $(D_1/D_0)/(C_1/C_0)$ \\ 
...
\medskip
\item
Yet, with a closed population or cohort, 
stability of exposure distribution may be unrealistic.
\pause
\medskip
\item
...
\begin{frame}
 ...
 at the outset. --  Serves as reference group for all cases. 
...
borrows some basics ideas from survey sampling.
\pause
\begin{itemize} 
\item
...
and CC with about same amounts of cases and controls.
\ei
\pause
\item Statistical modelling and valid inference \pause
\bi
\item[ ] Straightforward for both designs with appropriate software, now 
 widely available for CC, too 
% fitting
...
% be used when supplemented with additional tricks.
 ...
 for analytic batch, storage time, freeze-thaw cycle,  
  ...
  performed at different times than for cases $\to$ 
  differential quality \& misclassification.
 \ei
 \pause
