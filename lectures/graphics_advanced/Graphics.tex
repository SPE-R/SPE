...
\end{document}
 ...
 and have their own coordinates
  \item 
%Math on Plots and lattice plots topics are too advanced - MTP
...
\begin{frame}
  \frametitle{Grid and Lattice Graphics}
  \begin{itemize}
  \item Standard R graphics allow graphs to be arranged in an $m\times
    n$ gridded layout.
  \item The \code{grid} package allows arbitrary \emph{viewports} and
    create \emph{graph objects} (``grobs'') which can be modified
    before they are printed.
  \item The \code{lattice} package uses \code{grid} for a
    structural approach to multiframe graphs
  \item This is mostly compatible with S-PLUS's Trellis graphics
  \item Model formulas, \code{y\textasciitilde x|g1*g2*...}
  \item Shingles: Partially overlapping intervals used for
    conditioning plots
  \end{itemize}
\end{frame}
...
\begin{frame}
  \frametitle{Math on Plots}
  \begin{itemize}
  \item Sort of like TeX
  \item Works on unevaluated expressions (quote(alpha),
    expression(alpha))
  \item Special conventions: \code{\^},\code{[]} super/subscript,
    special names \code{alpha}, \code{sum}, \code{int}
  \item See \code{help(plotmath)} and \code{demo(plotmath)}
  \item Manipulating the unevaluated expressions (``Computing on the
    Language'') is useful, albeit somewhat tricky.
  \end{itemize}
\end{frame}
