...
\begin{frame}
  ...
  \item Based on S graphics, as described in ``The Blue Book''\footnote{Becker, Chambers and Wilks, 
...
\begin{frame}
  ...
  The \texttt{plot()} function is a {\bf generic} function.
  \begin{itemize}
  \item It does different things depending on what arguments you give it.
  \item The actual work is done by a {\bf method} function. See \texttt{methods(plot)}.
  \end{itemize}
...
\begin{frame}
  ...
  If the first argument to \code{plot()} is numeric, you get a scatter plot
  \begin{itemize}
  \item \code{plot(y)} Plot values of \code{y} against index ({\em i.e.} position in vector)
  \item \code{plot(x, y)} Plog values of \code{y} against corresponding values of \code{x}
  \end{itemize}
  Change the behaviour with the \code{type} argument
  \begin{itemize}
  \item \code{"l"} for \underline{l}ines
  \item \code{"p"} for \underline{p}oints (the default)
  \item \code{"b"} for \underline{b}oth
  \item plus  quite a few more...
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \begin{description}
  \item[barplot, hist] Bar plots and histograms
  \item[boxplot] Box plots
  \item[cdplot] Conditional density plots
  \item[dotchart] Dot plots
  \item[contour, image, persp] 3-D visualisation
  \item[mosaicplot] Mosaic plots
  \item[pie] Pie charts
  \end{description}
...
\begin{frame}
  \frametitle{Scoping in base graphics (1/2)}

  Base plotting functions have the same scoping rules as any other R
  function
  \begin{itemize}
  \item Look for arguments in the calling environment ...
  \item Then the workspace...
  \item Then the rest of the search path ...
  \end{itemize}

\end{frame}

\begin{frame}[fragile]
  \frametitle{Scoping in base graphics (2/2)}

  What if my variables are in a data frame?

  ~\\
  
  Use the \code{formula} method, which admits a \texttt{data} argument
  \begin{verbatim}
  plot(y ~ x, data=mydata)
  \end{verbatim}
  Or wrap the plotting function in a call to \texttt{with()}
  \begin{verbatim}
...
%\addtocounter{framenumber}{-1} %% workaround for beamer buglet
\begin{frame}[fragile]
  \frametitle{Second Base Graphics Demo}
  \begin{vcode}
x <- runif(50,0,2)
y <- runif(50,0,2)
...
\begin{frame}
   \frametitle{Grammar of Graphics in R}
   ...
   \item Described more fully by the \code{ggplot2} package author Hadley
      ...
      2009.
   \item Third edition (in progress) is available for free online at
      \url{https://ggplot2-book.org/}
   \item Built on top of the \texttt{grid} package
   \end{itemize}

\end{frame}

\begin{frame}
  \frametitle{How ggplot works}

  \begin{itemize}
  \item Start with a call to \texttt{ggplot} which defines
  \begin{itemize}
  \item The data frame that contains all variables
  \item The aesthetic mapping from variables to graphical parameters
  \end{itemize}
  \item Add additional graphical components with \texttt{+}
  \begin{itemize}
  \item Usually geometries (\texttt{geom\_point}, \texttt{geom\_line}, ...)
  \end{itemize}
  \item GGplot will combine all graphical elements together
  \end{itemize}

\end{frame}

\begin{frame}
  \frametitle{Scoping in ggplot}

  \begin{itemize}
  \item Unlike base graphics, ggplot looks for variables in a data frame
  which must be supplied as an argument (or piped in)
  \item The data frame must be ``tidy''
  \begin{enumerate}
  \item Every column is a variable
  \item Every row is an observation
  \item Every cell is a single value
  \end{enumerate}
  \item Getting data into the right ``tidy'' format is often more work
  than plotting
  \end{itemize}
