...
\begin{frame}[fragile]%{Follow-up and rates}
  ...
  \item Rates between multiple states (little boxes --- later)
  \end{itemize}
...
%----------------------------------------------------------------------
\begin{frame}{Statistical model for follow-up data}
\vspace*{-1ex}
  \begin{itemize}[<+->]
  \item \textbf{Data}:
    ...
    \item \ldots events (= change of status) and \textbf{observed} risk time:\\
  ...
  \item \textbf{Model} for occurrence rates:
    ...
    \item likelihood is a function of $\textcolor{blue}{\lambda}$ and
      \textcolor{forestgreen}{\textbf{data}}: $\ptxt{\textcolor{forestgreen}{data} $|$ model, $\lambda$}$
  ...
  $\textcolor{forestgreen}{d}\,\textcolor{blue}{\log(\lambda)} -
   \textcolor{blue}{\lambda \textcolor{forestgreen}{y}}$,
...
\textcolor{blue}{
 ...
 \put(240,37){\makebox(0,0)[lb]{$d \in (0,1)$}}
...
\end{align*}
\pause
---allows different rates ($\lambda_{\textcolor{forestgreen}{i}}$)
\textbf{between} intervals,\\ but assumes constant rates
\textbf{within} intervals
\end{frame}

%----------------------------------------------------------------------
\begin{frame}{Dividing time in small intervals requires:}
\pause
\textbf{time scale / origin}: date where the time scale is $0$: \pause
  \begin{itemize}
...
\pause
\textbf{Intervals:} How should time be subdivided:
 \begin{itemize}
  \item 1-year classes? 5-year classes? Equal length?
  \item ---short enough for assumption of constant rate in
each interval
  \end{itemize}

\pause \textbf{Aim:} Separate rate in each interval\\ ---mimicking
continuous time using short intervals with \textcolor{red}{constant rate}\\---time at the
beginning of interval used as \textcolor{red}{quantitative variable}.
...
\begin{frame}[fragile]{Follow-up intervals on several timescales}
  ...
  \begin{itemize}
    \item Age at entry
    \item Date of entry
    \item Time since diagnosis at entry\\
...
\onslide<5->{\alert<5->{\small \texttt{exitdat - injecdat}}}

\onslide<6->{One element of \code{entry} and \code{exit} must have
same name (here, \alert<6->{\texttt{dat}}).}
\end{frame}
...
> round(spl1, 1)
 age    dat  tfi lex.dur lex.Cst lex.Xst lex.id sex birthdat contrast injecdat volume
\alert<2>{22.2 1938.8  0.0    17.8       0       0      1   2   1916.6        1   1938.8     22}
\alert<2>{40.0 1956.6 17.8    20.0       0       0      1   2   1916.6        1   1938.8     22}
\alert<2>{60.0 1976.6 37.8     0.2       0       1      1   2   1916.6        1   1938.8     22}
\alert<3>{49.5 1945.8  0.0    10.5       0       0    640   2   1896.2        1   1945.8     20}
\alert<3>{60.0 1956.2 10.5     8.1       0       1    640   2   1896.2        1   1945.8     20}
\alert<4>{68.2 1955.2  0.0     1.4       0       1   3425   1   1887.0        2   1955.2      0}
\alert<5>{20.8 1957.6  0.0    19.2       0       0   4017   2   1936.8        2   1957.6      0}
\alert<5>{40.0 1976.8 19.2    15.3       0       0   4017   2   1936.8        2   1957.6      0}
...
\end{semiverbatim}
\renewcommand{\baselinestretch}{1.0}
...
\begin{frame}[fragile]{Likelihood for time-split data}
  ...
  \item So the likelihood from a single \textbf{person}
    \alert<4->{looks like} the likelihood from several
    \textcolor{forestgreen}{independent Poisson} variates
  \item \ldots but the data are \textcolor{red}{neither}
    \textcolor{forestgreen}{independent} \textcolor{red}{nor}
    \textcolor{forestgreen}{Poisson}
...
\begin{itemize}[<+->]
\item $d_{pi}$ --- events: \tcode{lex.Xst=="Dead" \& lex.Xst!=lex.Cst}
%     In the model as response: \tcode{lex.Xst==1}
\item $y_{pi}$ --- risk time: \tcode{lex.dur} (\tcode{dur}ation)\\
%     In the model as\\
...
\item Rate likelihood for \textbf{one person} is identical to a Poisson
  likelihood for \textbf{many independent} Poisson variates
...
%----------------------------------------------------------------------
\begin{frame}[fragile]{Fitting a simple model with \tcode{poisson}}
\begin{Schunk}
\begin{Soutput}
 ------------------------------------ 
 contrast         D        Y    Rate  
...
%----------------------------------------------------------------------
\begin{frame}[fragile]{Fitting a simple model with \tcode{poisreg}}
\begin{Schunk}
\begin{Soutput}
 ------------------------------------ 
 contrast         D        Y    Rate  
...
%----------------------------------------------------------------------
\begin{frame}[fragile]{Fitting a simple model with \tcode{glmLexis}}
\small
The wrapper \tcode{glmLexis} requires that \tcode{lex.Cst} and \tcode{lex.Xst}
are factors --- see \tcode{factorize}:
\vspace*{-1ex}
\begin{Schunk}
\begin{Sinput}
