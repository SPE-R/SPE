...
\begin{frame}{Cohort rates vs. population rates: SMR}
  ...
  \item Cohort rates proportional to reference rates, $\lambda_\text{p}$:\\
    ...
    age-bands.
  \item $D_a$ deaths during $Y_a$ person-years in age-band $a$ gives
    the likelihood:
 \begin{eqnarray*}
...
\begin{frame}
  ...
  \begin{itemize}[<+->]
  \item As for the rates, the SMR can be modelled using interval
    data.
  \item First column of response is $d_i$, the event indicator
    (\tcode{lex.Xst}).
  \item Second column the expected value for each piece of
...
\item Replace $Y$ with $E$ --- that's all! (\tcode{glmLexis} not usable)
