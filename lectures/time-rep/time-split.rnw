...
\textbf{between} intervals, but constant
...
\begin{frame}[fragile]{Likelihood for time-split data}
  ...
  \item So the likelihood from a single \textbf{person}
    ...
    Poisson variates
  \item \ldots but the data are \textcolor{red}{neither} independent \textcolor{red}{nor} Poisson
...
\item Rate likelihood for one person is identical to a Poisson
