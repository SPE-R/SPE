...
& &\onslide<2->{\textcolor{red}{\,=\,}0\,\textcolor{red}{ \log(\lambda) - \lambda} y_1}\\
  &\onslide<2->{\textcolor{red}{\times \operatorname{P}(\text{surv $\textcolor{red}{t_1}\rightarrow 
   \textcolor{red}{t_2}$} | \text{entry \textcolor{red}{$t_1$}})} }
...
%----------------------------------------------------------------------
\begin{frame}{Dividing time into bands:} 
...
%----------------------------------------------------------------------
\begin{frame}[fragile,allowframebreaks]{Follow-up data in \texttt{Epi} --- \texttt{Lexis} objects}
...
%     In the model as\\
%     offset $\log(y)$, \texttt{log(lex.dur)} or\\
%     part of response
\item Covariates are:
