...
%% For binary $X$, {standardization} or  {g-formula} applied  to predicted values.
\bi
\item
% Marginal {causal} risk differences, and  ratios are estimable from a logit-model,
...
% where $\text{expit}(u) = 1/[1+\exp(-u)]$ is the inverse of logit-function.
% \medskip 		
\item
...
% \begin{center}
$$ \lambda_x(t) = \lambda(t \mid X=x ) = \lim_{h\to 0}  \frac{P\{Y(t+h)=1 \mid X=x\}/h }{P\{Y(t)=1 \mid X=x\}}
. $$
% \end{center}
\item
