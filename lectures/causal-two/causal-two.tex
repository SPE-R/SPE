...
{\small\color{blue}
\begin{verbatim}
> probY.1 <- sum( 1*(dd$x==1)*dd$w*dd$y ) / sum( 1*(dd$x==1)*dd$w )
> probY.0 <- sum( 1*(dd$x==0)*dd$w*dd$y ) / sum( 1*(dd$x==0)*dd$w )
\end{verbatim}
}
...
\medskip
\item
The \textbf{hazard} of outcome at $t$ and the risk of outcome in $(0,t]$ for those factually 
exposed to level $X=x$, $x=1,0$ (See Janne's lecture on Wednesday).
\begin{align*}
 \lambda_x(t) & = \lambda(t \mid X=x ) \approx 
 \frac{1}{h} P\{Y(t+h)=1 \mid Y(t)=0, X=x\}/h, \\
