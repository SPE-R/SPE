...
\begin{itemize}
\item[--] What is the 10-year risk % $\pi^1$ 
of the outcome,
\underline{if everybody in P were exposed} to HT, as compared with % $\pi^0$, 
the risk
\underline{if nobody} were exposed?
\pause
\medskip
\item[--] What is the 10-year risk % $\pi^1_1$ 
of the outcome,
\underline{among those factually exposed to HT} in P,  
as compared with the \\ risk  % $\pi^0_1$, \\
among those factually exposed, \underline{had they not been} exposed?
\end{itemize}
...
subset of the target population {\bf factually exposed} to level $X=x$:
...
\bi
\item
The associational quantities above are \textbf{marginal}; 
They are \\ not conditioned on (or stratified by) any {covariate} -- such as sex, age, etc.
\pause
\medskip
\item
...
both $X=x$ \underline{and} $Z=z$, where $x=0,1$ but $z$ may have multiple values.
\pause
\medskip
\item
...
\item % [$\Rightarrow$]
 Marginal risks,
$\pi_1, \pi_0$,  associational contrast $\tau = \pi_1 - \pi_0$,
and conditional associational contrasts 
...
indicate (1/0) the event to occur in the risk period, \\
\medskip
...
\begin{center}
$  \pi^x = P \{ Y^{X=x} = 1 \} = E(Y^{X=x}), \quad x=0,1. $ %% = P\{ T^x \leq t  \} $$
\end{center}
\medskip
\item
{\bf Marginal causal contrasts} \\ %% , suppressing dependence on time.
\medskip
\begin{itemize}
{\normalsize
\item[--]
...
\end{itemize}
\medskip
{\small (see \href{https://doi.org/10.48550/arXiv.2303.16008}{\color{blue}Colnet et al. (2023)}
``\dots Which causal measure is easier to generalize?'')}
\medskip
\item[NB.] Superscript $x$ (1 or 0) refers to 
potential or counterfactual exposure, while \\
subscripts refer to factual exposure groups. 
% \pause
% \medskip
% \item[\textbf{NB}.]
% Alternative notation: 
% \href{https://doi.org/10.2202/1557-4679.1203}{\color{blue}Judea Pearl's (2010)} {\bf do-operator} 
% \begin{center}
...
\begin{center}
 \pi^x  = E( Y^{X=x})  % = E_Z[E_Y(Y|X=x, Z)] \\
    = \sum_z P \{ Y=1 \mid X= x, Z=z \} P\{ Z=z\} , \\
\end{center}		
%      & = E(Y^{X=x}) = E_Z[E(Y|X=x, Z)] = \int E(Y|X=x, Z=z) dF_Z(z)
\medskip
\pause
\item
Causal contrasts of risks $\tau^c$, $\phi^c$, $\psi^c$	are obtained from  $\pi^1$
and $\pi^0$.
\pause
\medskip
\item 
...
and $X$ is independent of $Z, U, \dots $, measured and unmeasured confounders.
\pause
\medskip
\item
...
$ \pi^x = \sum_z \pi_{xz} P\{ Z=z\}$, with {\bf weights} $P\{ Z=z\}$ from whole population: 
\begin{align*}
...
$$ \pi_z^x = P\{ Y^{X=x} = 1 \mid Z=z \},\quad x=0,1, \ \text{ for each factual }z$$ 
\item
...
%% = P\{ Y^{X=1} = 1 \mid Z=z \} - P\{ Y^{X=0} = 1 \mid Z=z \}, \\
  %% = P\{ Y^{X=1} = 1 \mid Z=z \} / P\{ Y^{X=0} = 1 \mid Z=z \}
\end{center}	
\pause
\medskip
...
\pause
\item
Different standard populations for ATT, ATU, and for the marginal contrast, a.k.a.
...
\medskip
\item
For ATT, we have the observed risk $\pi^1_1 = \pi_1$ = \textbf{0.40}. -- The
expected risk:
\begin{center}
$\pi^0_1 = \sum_z \pi_{0z} P\{Z=z|X=1\}$ = 0.25$\times$0.667 + 0.10$\times$0.333 = \textbf{0.20}.
\end{center}
Thus, ATT = 0.40 $-$ 0.20 = \textbf{0.20}.
\pause
\medskip
\item
...
\item
 In the spirit of \textbf{generalized linear models}, let vector  
  ...
  The \textbf{linear predictor} $\eta_i$ is specified: \\
	\begin{center}
 $ \eta_i = \alpha + \beta x_i + \gamma^{\small\text{T}}z_i $
% =  \beta_1 x_{i1} + \dots + \beta_p x_{ip} $$ 
\end{center}
-- assuming so far no {\bf interactions} or {\bf effect modifications} 
\pause
\medskip
\item
...
log of \textbf{conditional} risk odds ratio (OR),  $\psi_z^c$, \ \ \ -- " -- \\
{\bf NB.} This is different from \textbf{marginal} OR due to \textbf{non-collapsibility}.
}
\ei
\pause
\item
...
It may contain product terms allowing modification
\begin{center}
...
the principle of {\bf regression standardization} or {\bf g-formula}:
...
\begin{frame}
\frametitle{\large R packages, computation of SEs and CIs}
\bi
\item
In practicals we use package {\tt stdReg2} 
{\small(\href{https://doi.org/10.1007/s10654-016-0157-3}{\color{blue}Sj{\"o}lander 2016}, 
 \href{https://doi.org/10.1016/j.annepidem.2025.04.002}{\color{blue}Sachs et al. 2025})}
\medskip
\item
Statistical inference is based on M-estimation, sandwich formula
for the covariance matrix of model parameters, delta-method etc. \\
{\small (\href{https://www.jstor.org/stable/3087324}{\color{blue}Stefanski \& Boos 2002})} 
\medskip
\item  
Bootstrapping is also possible. This involves some issues, though, and is 
computationally demanding.
\medskip
\item Other packages: {\tt marginaleffects, gfoRmula}, \dots 
\ei
\end{frame}
...
\bi
\item
With IPW, counterfactual risks in the whole population 
are estimated as weighted averages of the outcome in the two exposure groups
$$
...
\pause
\item
For causal contrasts {\bf among the treated} (ATT), use
 \textbf{treated weights}:
\begin{center} 
   $W_i = 1$ for $X_i=1$, and $W_i = \text{PS}_i/(1-\text{PS}_i)$
for $X_i=0$. 
\end{center}
\pause
\medskip
...
\bi
\item
In our practicals we use package {\tt PSweight} 
{\small (\href{https://doi.org/10.32614/RJ-2022-011}{\color{blue}Zhou et al. 2022})}
\medskip
\item
Statistical inference is again based on M-estimation, nuisance-adjusted 
sandwich covariance matrix and delta-method etc. 
\medskip
\item
Bootstrapping is also possible with same issues. 
\medskip
\item
Diagnostic tables and graphs for covariate balance
assessment.
\medskip
\item
...
\medskip
\item 
Other packages: {\tt Weightit, ipw, survey, twang}, \dots
\ei
\end{frame}

\begin{frame}
\frametitle{Double robust (DR) estimation and machine learning methods}
\bi
\item
...
R packages {\tt PSweight, AIPW}, \dots
\medskip
...
\medskip
\item
The \textbf{hazard} of outcome at $t$, $\lambda_x(t)$, and the {\bf risk}
 of outcome during $(0,t]$, $\pi_x(t)$, for those factually 
exposed to level $X=x$, $x=1,0$: %% (See Janne's lecture on Wednesday).
\begin{align*}
 \lambda_x(t) & = \lambda(t \mid X=x ) \approx 
  P\{Y(t+h)=1 \mid Y(t)=0, X=x\}/h, \\
...
%% \begin{center}
   $ \pi^x(t) = P\{ Y^{X=x}(t) \} = P\{ T^{X=x} \leq t \}, x=0,1 $,
	the contrasts of which are causally interpretable. 
%% \end{center}
\pause
\medskip
\item
...
\bi
\item
 Counterfactual % survival probabilities $1-\pi^x(t)$ and 
