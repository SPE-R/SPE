...
\begin{frame}
  \frametitle{Predicting birth weight from gestational age}
...
\begin{frame}
  \frametitle{A cubic model for the births data}

  \centering
  \includegraphics[width=6cm]{figures/births-poly.pdf}
...
\begin{frame}
  ...
  \begin{itemize}
  \item{\bf preterm} $<37$ weeks
  \item{\bf early term} 37-39 weeks
  \item{\bf full term} 39-41 weeks
  \item{\bf late term} 41-42 weeks
  \item{\bf post term} $\geq 42$ weeks
  \end{itemize}
...
\begin{frame}
  ...
  \centering
  \includegraphics[width=6cm]{figures/births-step1.pdf}
...
\begin{frame}
  ...
  \begin{itemize}
  \item{\bf extremely preterm} $<28$ weeks
  \item{\bf very preterm}  28-32 weeks
  \item{\bf moderate to late preterm} 32-37
  \end{itemize}
...
\begin{frame}
  ...
  \centering
  \includegraphics[width=6cm]{figures/births-step2.pdf}
...
\begin{frame}
  ...
  \item Many diseases are not discrete. Instead there is an underlying
    continuum of increasing severity.
...
\begin{frame}
  \frametitle{Example: hypertension}
  
  In 2017, the American College of Cardiology \& American Heart Association
  changed the definition of hypertension
  \begin{description}
  \item[old definition] $\geq 140/90$
  \item[new definition] $\geq 130/80$
  \end{description}

  This increased the estimated prevalence of hypertension in the USA from 32\% to 46\%
  adding 31 million new cases~\footnote{Muntner et al 2018: \url{https://pubmed.ncbi.nlm.nih.gov/29133599/}}
...
\begin{frame}
  ...
  \begin{itemize}
  \item There are fixed points on the x-axis called {\em knots}
...
\begin{frame}
  \frametitle{Dose response with error}
    ...
    \begin{column}{6cm}
      ...
      point but always measure with error.

      ~\\
      
      An interpolating spline is not useful in this case. It fits
      the curve to the noise as well as the underlying dose-response.
...
\begin{frame}
  \frametitle{Dose response with error}
    ...
    \begin{columns}
    \begin{column}{6cm}
      \includegraphics[scale=0.4]{figures/smooth2.png}
    \end{column}
    \begin{column}{6cm}
      We know how to fit a maximally smooth (linear) curve
      to data with errors with linear regression.
...
\begin{frame}
  \frametitle{Maximum fit vs Maximum smoothness}

  \begin{columns}
  \begin{column}{6cm}
    \includegraphics[scale=0.4]{figures/smooth1.png}
  \end{column}
  \begin{column}{6cm}
    \includegraphics[scale=0.4]{figures/smooth2.png}
  \end{column}
...
\begin{frame}
  ...
  Assuming $f(x)$ is a spline with fixed knots, minimize
  \[
...
\begin{frame}
  ...
  \begin{columns}
    ...
    \begin{column}{6cm}
      ...
      \item Smoothing reduces this to 1.88 effective degrees of freedom
      \end{itemize}
    \end{column}
  \end{columns}
...
\begin{frame}
  ...
  \begin{columns}
    \begin{column}{6cm}
      \includegraphics[scale=0.4]{figures/gam-nonsmoothed.png}
    \end{column}
    \begin{column}{6cm}
      \begin{itemize}
...
\begin{frame}
  \frametitle{Penalized vs unpenalized splines}

  \begin{columns}
    \begin{column}{6cm}
      \includegraphics[scale=0.4]{figures/gam-7.png}
    \end{column}
    \begin{column}{6cm}
      \includegraphics[scale=0.4]{figures/gam-nonsmoothed.png}
    \end{column}
...
\begin{frame}
  \frametitle{A penalized spline model for the births data}

  \centering
  \includegraphics[width=6cm]{figures/births-gam.pdf}
