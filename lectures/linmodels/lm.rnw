...
%%  LaTeX script for the lecture "Fitting generalized linear models"
%%  in SPE, Saturday 18 June, 2016 -- Esa Läärä
...
\medskip 
\item Fitting a model and extracting results.
\medskip
...
<<par, echo=F, results=hide>>=
par(mar= c(5,4,1,1)+0.1)
@
\frametitle{Birth weight and gestational age}
\ \\[-3em]
\includegraphics[height=0.8\textheight,keepaspectratio]{lm-bw-by-gw}
\ \\[-2em]
{\small
<<bw-by-gw, fig = T, echo=T>>=
...
\ \\[-2em]
<<bw-by-gw-diag, fig=T, height=6, width=12>>=
par(mfrow=c(1,2))
...
= difference between level 2 {\it vs.} reference level 1 of {\tt hyp}
\ei
\vfill
\end{frame}
...
\medskip
\item
Main effect of {\tt hyp} = $-77.3$ is the % effect of {\tt hyp} 
difference between
...
<<mp3plot, echo=F, results=hide, fig = T >>=
pr.mp3 <- predict( mp3, newdata=nd, interval="conf" )
with( births, plot( bweight ~ gestwks, xlim = c(24, 45), cex.axis= 1.5, cex.lab = 1.5 )  )
matlines( nd$gestwks, pr.mp3, lty=1, lwd=c(3,2,2), col=c('red','blue','blue') )
@
}
\end{frame}
...
\ \\[-2em]
{\small
<<bw-by-gw-prsp, results=hide >>=
mNs10 <- lm( bweight ~ Ns( gestwks, 
      knots = seq(25, 43, by = 2)), data = births)
@
}
<<mNs10-plot, echo=F, results=hide, fig = T >>=
pr.Ns10 <- predict( mNs10, newdata=nd, interval="conf" )
with( births, plot( bweight ~ gestwks, xlim = c(24, 45), cex.axis= 1.5, cex.lab = 1.5 )  )
matlines( nd$gestwks, pr.Ns10, lty=1, lwd=c(3,2,2), col=c('red','blue','blue') )
@
\ \\[-2em]
\begin{itemize}
...
<<mNs2-plot, echo=F, fig=T, results=hide>>=				
pr.Ns2 <- predict( mNs2, newdata=nd, interval="conf" )
with( births, plot( bweight ~ gestwks, xlim = c(24, 45), cex.axis= 1.5, cex.lab = 1.5 )  )
matlines( nd$gestwks, pr.Ns2, lty=1, lwd=c(3,2,2), col=c('red','blue','blue') )
@
\ \\[-2em]
\begin{itemize}
...
\ \\[-3em]
{\small
<<bw-by-gw-pensp, results=hide >>=
library(mgcv)
mpen <- gam( bweight ~ s(gestwks), data = births)
@
}
<<mPen-plot, echo=F, fig=T, results=hide>>=				
pr.pen <- predict( mpen, newdata=nd, se.fit=T)
with( births, plot( bweight ~ gestwks, xlim = c(24, 45), cex.axis= 1.5, cex.lab = 1.5 )  )
matlines( nd$gestwks, cbind(pr.pen$fit, pr.pen$fit - 2*pr.pen$se.fit, pr.pen$fit + 2*pr.pen$se.fit),  
  lty=1, lwd=c(3,2,2), col=c('red','blue','blue') )
@
\ \\[-2em]
\begin{itemize}
\item Looks quite nice.
\item Model degrees of freedom $\approx 4.2$;  \\ 
...
\ \\[-2em]
<<bw-by-gw-spline-diag, results=hide, fig=T, height=10, width=12>>=
