...
\medskip
\item[--] Events in person-time, or incidence rate. 
% \medskip
...
$\to$ 
 Simple {\bf linear regression model} $Y=\alpha + \beta x$ is fitted.
\begin{semiverbatim}
> m <- lm(bweight ~ gestwks, data=births)
\end{semiverbatim}
\begin{itemize}
...
\medskip
\item
\verb|coef(m)| -- $\widehat\alpha$ and $\widehat\beta$ only (see above)
%% {\small
%% \begin{verbatim}
%% (Intercept)     gestwks 
%%     -4489.1      197.0 
...
\medskip
\item
\verb|ci.lin(m)[,c(1,5,6)]| -- estimates \& confidence limits 
{\small
\begin{verbatim}
            Estimate    2.5%   97.5%
...
at some meaningful reference value, for instance 40 weeks: 
% using e.g.
% the {\bf insulate} operator {\tt I()} for explicit  
% transformation of an original term.
\bi 
\item {\small \verb|births$gw_40 <- births$gestwks - 40|
\item {\small \verb|mi2 <- lm(bweight ~ hyp*gw_40, ...)|}
{\small  
\begin{verbatim}
                     Estimate   2.5%  97.5%
...
<<mPen-plot, echo=FALSE, fig=TRUE, results=hide>>=				
pr.pen <- predict( mpen, newdata=nd, se.fit=TRUE)
with( births, plot( bweight ~ gestwks, xlim = c(24, 45), cex.axis= 1.5, cex.lab = 1.5 )  )
