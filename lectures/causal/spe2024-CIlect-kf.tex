...
\begin{frame}
	...
	\begin{block}{Possible questions \ldots}
		...
		\item If, in one of the two \sini{``parallel universes''} we could eliminate access to white bread (no-one gets any) and 
		in the other, have everyone eating, say 200 grams of white bread per day -- would the average weight  differ (after a while)? 
	\end{enumerate}
			\end{block}
...
\begin{frame}
	...
	\begin{itemize}
\item Suppose there is an outcome variable $Y$ (weight) and exposure variable $X$ (white bread consumption). 
A regression model would estimate \sini{the conditional expectation} $E(Y|X)$. 
\item We might find: $$E(Y|X=5) \ne E(Y|X=10).$$
\item With a binary outcome $Y$, logistic regression may yield: 
%enables such inferences about probabilities ($Y$: disease, coded as 1-present, 0-absent; $X$-and exposure, also 1-present, 0-absent), for instance, concluding that: \\
$$P(Y=1|X=1)\ne P(Y=1|X=0).$$
\item Both are statements about differences between groups of individuals. 
\end{itemize}
\sini{The groups that differ by the value of X, may also differ in many other ways. }
\end{frame}
