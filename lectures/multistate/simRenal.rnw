...
setwd("/bendix/teach/SPE/git/SPE/lectures/multistate")
library(Epi)
library(mgcv)
library(foreign)
renal <- read.dta("../../pracs-book/data/renal.dta")
# renal <- read.dta("https://raw.githubusercontent.com/SPE-R/SPE/master/pracs/data/renal.dta")
subset(renal, dob > 2000)
renal <- transform(renal,
                   sex = factor(renal$sex,
                                labels = c("M", "F")),
                   dob = ifelse(dob > 2000,
                                dob - 100,
                                dob))
...
summary(Lc)
clr <- c("orange", "forestgreen")
clr <- c(clr, adjustcolor(rev(clr), alpha = 0.3))
