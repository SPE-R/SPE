...
%----------------------------------------------------------------------
\begin{frame}[fragile]{Cutting follow-up at remission: \code{cutLexis}}
\renewcommand{\baselinestretch}{0.8}
\footnotesize
\begin{verbatim}
...
%----------------------------------------------------------------------
\begin{frame}[fragile]{Showing states and FU: \code{boxes.Lexis}}
\renewcommand{\baselinestretch}{0.8}
\footnotesize
\begin{verbatim}
...
%----------------------------------------------------------------------
\begin{frame}[fragile]{Cutting follow up at events: \code{cutLexis}}
\renewcommand{\baselinestretch}{0.8}
\footnotesize
\begin{semiverbatim}
> Lc <- cutLexis( Lr, cut = Lr$dor,
...
\begin{frame}[fragile]{Likelihood for a general MS-model}
  ...
  \item Possible to fit models to \code{Lexis} objects

    \begin{itemize}
    \item separately for each transition
    \item jointly for transitions \textbf{from different} states \textbf{to the same} state
    \item jointly for transitions \textbf{from the same} state \textbf{to different}
      states:\\ normally not meaningful, so \alert{don't!}
    \end{itemize}
...
$\lambda$ (\code{lambda}),
$\mu_\text{NRA}$ (\code{mu.nra}) and
$\mu_\text{rem}$ (\code{mu.rem}) are known at any age (stored in vectors)
...
\code{c.mort.rem} should have an extra argument---technically very
complicated
