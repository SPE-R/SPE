...
%----------------------------------------------------------------------
\begin{frame}[fragile]
  \frametitle{Splitting states: \texttt{cutLexis}}
\renewcommand{\baselinestretch}{0.8}
\footnotesize
\begin{semiverbatim}
> Lc <- cutLexis( Lr, cut=Lr$dor,
...
+               new.state="Rem",
+        precursor.states="NRA",
+            \alert<1->{split.states=TRUE} )
> summary( Lc )

Transitions:
     To
From  NRA Rem ESRD\alert<2>{ ESRD(Rem)}  Records:  Events: Risk time:  Persons:
  NRA  24  29   69\alert<2>{         0}       122       98     824.77       122
  Rem   0  24    0\alert<2>{         8}        32        8     259.90        32
  Sum  24  53   69\alert<2>{         8}       154      106    1084.67       125
\end{semiverbatim} %$
\normalsize
\renewcommand{\baselinestretch}{1.0}
\end{frame}
