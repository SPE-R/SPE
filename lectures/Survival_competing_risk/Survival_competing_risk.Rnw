...
\author{Janne Pitk\"aniemi (initial slides EL) \newline \newline }
\institute{Finnish Cancer Registry \newline \newline
\normalsize Statistical Practice in Epidemiology (2019, Tartu) }
\date{}
...
<<km,echo=FALSE,eval=TRUE,fig=TRUE>>=
par(mfcol=c(1,1))
plot(survfit(orca$suob~orca$sex), ylab="Proportion",xlab="Time (years)",lwd=c(2,1.5,1.5,2,1.5,1.5),lty=c(1,3,3,1,3,3),conf.int = T,col=c("red","blue"),main="Estimated survival (95% CI) and CDF")
...
{\scriptsize 
<<boxplot,echo=FALSE,eval=TRUE,fig=TRUE>>=
library(Epi)
...
plotCIF(cif1, 1, main = "CIF for cancer death")
lines(km1$time,1-km1$surv,lty=c(2),type="s")
lines(cif1$time,1-cif1$pstate[,2],lty=c(1),type="s")
lines(km2$time,km2$surv,lty=c(2),type="s")

index15<-which(floor(cif1$time)==15)[1]-1
text(15,0.1,paste("P(death cancer)= \n",round(cif1$pstate[index15,1],4.4)))
text(15,0.8,paste("P(death other)= \n",round(cif1$pstate[index15,2],4.4)))
text(5,0.5,paste("P(alive)=",round(cif1$pstate[index15,3],4.4)))
abline(v=15,lty=3)
