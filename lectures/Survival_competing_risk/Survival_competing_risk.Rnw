...
\begin{itemize}
\item
\texttt{Surv(time, event)} \newline
creates a \textbf{survival object} assuming that all start at 0, containing pairs $(y_i, \delta_i)$,
 \medskip
 \item
\texttt{Surv(entry, exit, event)} \newline
 creates a survival object from
  \texttt{entry} and \texttt{exit} times, % \& event indicator,
  %\pause
  \medskip
\item
\texttt{survfit(Surv()} $\sim$ \texttt{x)} \newline
creates a \textbf{survfit} object
...
% applied to \texttt{survfit} object \texttt{sfo}
 plot method for survival curves and related graphs, 
  \medskip
\item
\texttt{coxph(Surv()} $\sim$ \texttt{x1 + x2)} \newline
fits a Cox model
...
cB8  <- c("#000000", "#E69F00", "#56B4E9", "#009E73", 
          "#F0E442", "#0072B2", "#D55E00", "#CC79A7") #suitable colorset
@
...
par(mfcol=c(1,1))
plot(survfit(orca$suob~orca$sex), ylab="Proportion",xlab="Time since diagnosis (years)",lwd=c(2,1.5,1.5,2,1.5,1.5),lty=c(1,3,3,1,3,3),conf.int = T,col=c(cB8[7],cB8[6]),main="Estimated survival (95% CI)")
text(3,0.9,"KM for S(t)")
legend(x=10, y=1,
            c("Female","Male"),
            col=c(cB8[7],cB8[6]), lty=1:1,lwd=c(2,2))
@
...
stackedCIF(cif1,xlim=c(0,20),main="Stacked CIF",
           col = c("black"),
           fill=c(cB8[3],cB8[4],cB8[7]))
text( 5, 0.10, "Oral ca death ", pos = 4)
text( 4, 0.45, " Other death ", pos = 4)
text( 1, 0.80, " Alive ", pos = 4)
...
text(17.5,0.6,paste(round(cif1$pstate[index15,3],2.2)),cex=1.2)
# 
...
plotCIF(cif1, 1, main = "CIF for cancer death",lty=c(2,1),col=c(cB8[7],cB8[3]),ylim=c(0,0.6))
legend(x=10, y=1,
            c("Female","Male"),
            col=c(cB8[7],cB8[3]), lty=2:1,lwd=c(2,2))

plotCIF(cif1, 2, main = "CIF for other death",lty=c(2,1),col=c(cB8[7],cB8[3]),ylim=c(0,0.6))
@
...
Thus $e^{\beta_j} = \text{HR}_j$ = \textbf{hazard ratio} or relative rate
 associated with a unit change in covariate $X_j$.
 \newline \newline
Hazard ratios should not be interpreted as causal estimands without proper causal inference (lectures by Esa)
