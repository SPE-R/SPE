...
\normalsize Statistical Practice in Epidemiology (2023, Tartu) }
\date{}
...
#setwd("C:/Users/janne.pitkaniemi/SPE2018")
#orca<-download.file(
#  url = 'https://raw.githubusercontent.com/SPE-R/SPE/master/pracs/data/oralca2.txt',
#  destfile = 'oralca2.txt'
#)
...
par(mfcol=c(1,1))
plot(survfit(orca$suob~orca$sex), ylab="Proportion",xlab="Time since diagnosis (years)",lwd=c(2,1.5,1.5,2,1.5,1.5),lty=c(1,3,3,1,3,3),conf.int = T,col=c("red","blue"),main="Estimated survival (95% CI)")
text(3,0.9,"KM for S(t)")
legend(x=10, y=1,
            c("Female","Male"),
            col=c("red","blue"), lty=1:1,lwd=c(2,2))
@
...
oral_cif<-data.frame(cbind(cif1$time,cif1$pstate))
colnames(oral_cif)<-c("time","alive","can_death","other")
plot(oral_cif$time, oral_cif$alive,type="l",xlab = "time",ylab="CIF")
lines(oral_cif$time, 1-oral_cif$can_death,type="l")


index15<-which(floor(oral_cif$time)==15)[1]-1
text(5,0.1,paste("P(alive)"))
text(12,0.5,paste("P(other)"))
text(10,0.8,paste("P(oral)"))
abline(v=15,lty=3)
text(20,0.05,paste(round(oral_cif[index15,2],3.2)),cex=2)
text(20,0.75,paste(round(oral_cif[index15,3],3.2)),cex=2)
text(20,0.4,paste(round(oral_cif[index15,4],3.2)),cex=2)
...
plotCIF(cif1, 1, main = "CIF for cancer death",lty=c(2,1),col=c("red","blue"),ylim=c(0,0.6))
legend(x=10, y=1,
            c("Female","Male"),
            col=c("red","blue"), lty=2:1,lwd=c(2,2))

plotCIF(cif1, 2, main = "CIF for other death",lty=c(2,1),col=c("red","blue"),ylim=c(0,0.6))
@
...
The M/F contrast visible only after age-adjustment.(43\% higher in males).
\normalsize
...
% 
% {\footnotesize
% <<em3,echo=FALSE,eval=TRUE,fig=TRUE,height=11,width=14>>=
% 
% par(mfrow=c(1,3),lwd=2,cex=1.4)
% 
% x <- fotcut
% y <- ci.exp(obs.mort)[,1]*100
% plot(x,c(y[1],y),type="n",xlim=c(0,5),ylim=c(0,37),ylab="Observed mortality /100 py",xlab="")
% segments(x[-length(x)],y[-length(x)],x[-1],y[-length(x)],lwd=4) 
% abline(h=0)
% 
% y <- ci.exp(obs.mort)[,1]*100-ci.exp(excess.mort)[,1]*100
% segments(x[-length(x)],y[-length(x)],x[-1],y[-length(x)],col="violet",lwd=4) 
% mtext("Observed and population",side=3,cex=1.5)
% #legend("topright",c("Total mortality","Population mortality"),col=c("black","gray"),lty=1,lwd=2,y.intersp = 2,seg.len = 3,text.width=3.3)
% 
% y <- ci.exp(excess.mort)[,1]*100
% plot(x,c(y[1],y),type="n",xlim=c(0,5),ylim=c(0,37),ylab="Excess mortality /100 py",xlab="Time since diagnoses (years)")
% #,ylab="Excess mortality per 100 person-years",xlab="Time since diagnoses (years)")
% segments(x[-length(x)],y[-length(x)],x[-1],y[-length(x)],lwd=4) 
% abline(h=0)
% mtext("Excess",side=3,cex=1.5)
% 
% y <- ci.exp(relative.mort)[,1]
% plot(x,c(y[1],y),type="n",xlim=c(0,5),ylim=c(0,11),ylab="Relative mortality",xlab="")
% #,ylab="Relative mortality",xlab="Time since diagnoses (years)")Excess mortality per 100 person-years
% segments(x[-length(x)],y[-length(x)],x[-1],y[-length(x)],lwd=4) 
% abline(h=1)
% mtext("Relative",side=3,cex=1.5)
% 
% @
% }
% 
