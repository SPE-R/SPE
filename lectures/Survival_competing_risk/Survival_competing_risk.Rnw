...
<<KM_plot,eval=TRUE,fig=TRUE,echo=FALSE>>=
library(survival)
orca<-read.table("../../data/oralca2.txt",header = TRUE)
plot(survfit(Surv(orca$time,orca$event>0)~1,data=orca))
title("Estimated survival",xlab="Time (years)",ylab = "Proportion")
lines(survfit(Surv(orca$time,orca$event>0)~1,data=orca))
@
