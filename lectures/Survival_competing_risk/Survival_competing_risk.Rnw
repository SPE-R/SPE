...
\normalsize Statistical Practice in Epidemiology (2026, Tartu) }
\date{}
...
km1 <- survfit( Surv(time, 1*(event > 0) ) ~ 1, data = orca)
Surv(orca$time, 1*(orca$event > 0))[1:7]
km1              #  brief  summary
@
}
...
{\scriptsize
<<or1, echo=TRUE,eval=TRUE>>=
res<-summary(km1,data.frame=TRUE)     #  detailed KM-estimate
round(res[1:10,c(1:7,9:10)],3)
@
}
...
par(mfcol=c(1,1))
plot(survfit(Surv(orca$time, 1*(orca$event > 0) )~orca$sex), 
     ylab="Proportion",xlab="Time since diagnosis (years)",lwd=c(2,1.5,1.5,2,1.5,1.5),lty=c(1,3,3,1,3,3),conf.int = T,col=c("#CC79A7","#56B4E9"),main="Estimated survival (95% CI)")
...
#knitr::opts_chunk$set(message = FALSE)
 orca.lex <- Epi::Lexis(exit = list(stime = time), 
            exit.status = factor(event, 
    labels = c("Alive", "Oral ca. death", "Other death") ),
                  data = orca)
Epi::boxes(orca.lex,show.Y = TRUE,show.R=TRUE,scale.R=1000,boxpos=TRUE,show.BE=TRUE)
...
<<>>=
cm0 <- coxph( Surv(time, 1*(event > 0) ) ~ sex, data = orca)
ci.exp(cm0)
@
...
<<>>=
cm0 <- coxph( Surv(time, 1*(event > 0) ) ~ age+sex, data = orca)
ci.exp(cm0)
@
