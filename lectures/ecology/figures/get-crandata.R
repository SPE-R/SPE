...
CRAN_page <- function(...) {
    ...
    file.path("https://cloud.r-project.org/src/contrib", ...)
}
...
library(reticulate)
## Use virtual environment previously created with shell commands:
##  python3 -m venv ./venv
...
## Look up release date of first version & number of releases
##
## This takes a while! For every package we need to inspect its archive directory
## to get the date stamp of its earliest release.
##
## If this stops due to, e.g.  Bad Gateway error then you can just
## rerun these lines and it carries on where it left off.
