...
\item (\emph{Optional, esoteric}) We can produce a slightly more fancy
  Lexis diagram. Note that we have a $x$-axis of 40 years, and a
  $y$-axis of 80 years, so when specifying the output file adjust the
  \emph{total} width of the plot so that the use of \texttt{mai} (look
  up the help page for \texttt{par}) to specify the margins of the
  plot so that it leaves a plotting area twice as high as wide. The
  \texttt{mai} argument to \texttt{par} gives the margins in inches,
  so the total size of the horizontal and vertical margins is 1 inch
  each if we specify \texttt{mai=c(3,3,1,1)/4}, to which we add 80/9
  in the height, and 40/9 in the horizontal direction, each giving
  exactly 9 years per inch in physical size.
  
\item Now make a Cox-regression analysis of the endpoint ESRD with the
  variables sex and age at entry into the study, using time since
  entry to the study as time scale (and therefore \texttt{lex.dur} the
  time of exit).
<<>>=
library( survival )
...
You may want to explore the \texttt{coxph.Lexis} function which is
just a wrapper for \texttt{coxph} using the special structures in a
\texttt{Lexis} object:
<<>>=
ml <- coxph.Lexis( Lr, formula = tfi ~ I(age/10) + sex )
ci.exp(ml)
@ %
...
\item The main focus of the paper was to assess whether the occurrence of
  ...
  \texttt{Lexis} object, where we introduce a remission state
  \texttt{Rem}.  You must declare the \texttt{NRA} state as a
  precursor state, i.e. a state that is \emph{less} severe than
  \texttt{Rem} in the sense that a person who see a remission will
  stay in the \texttt{Rem} state unless he goes to the \texttt{ESRD}
  state. Also use \texttt{split.state=TRUE} to have different ESRD
  states according to whether a person had had remission or not prioer
  to ESRD. The statement to do this is:
<<>>=
...
c(5,7,9) )}, or other numbers). Explain what each record represents.
...
\item (\emph{Optional, esoteric}) Plot a Lexis diagram where different
  coloring is used for different segments of the follow-up. The
...
\item Try to use \texttt{coxph.Lexis} to fit this model --- read the
  documentation for the function.
<<>>=
mt <- coxph.Lexis( Lc, formula = 
                       tfi ~ sex + I((doe-dob-50)/10) + (lex.Cst=="Rem") )
@ %
What happens if you use current age as the baseline timescale instead?
...
@ %
In the \texttt{popEpi} package there is a function \texttt{splitMulti}
that does the same, but with a simpler syntax (and much faster for
lareg datasets):
<<>>=
library(popEpi)
sLc <- splitMulti( Lc, tfi=seq(0,30,1/12) )
@ %
Read the help page for \texttt{splitMulti} to see the features.
...
ci.exp( mp )
@ %
As for the \texttt{coxph} there is also a wrapper for \texttt{glm} in
multistate model. Per default it will model all transition \emph{to}
any absorbing state (which is what we want in this case):
<<>>=
mp <- glm.Lexis( sLc, formula= ~ Ns( tfi, knots=c(0,2,5,9,14) ) +
                      sex + I((doe-dob-40)/10) + I(lex.Cst=="Rem") )
ci.exp( mp )
@ %
...
\item Try instead using the \texttt{gam} function from the
  \texttt{mgcv} package --- a function that allows the smoother
  \texttt{s}, that optimizes the number as well as the location of the
  knots:
<<>>=
library( mgcv )
mx <- gam( cbind(lex.Xst %in% EP,lex.dur) ~ s( tfi, k=10 ) +
           sex + I((doe-dob-40)/10) + I(lex.Cst=="Rem"),
           family = poisreg, 
             data = sLc )
...
@ %
There is also a wrapper for \texttt{gam} fitted to \texttt{Lexis} objects:
<<>>=
...
@ %
How large is the difference in estimated regression parameters?
...
\item The model has the same assumptions as the Cox-model about
  ...
  general cross-validation. Try to look at the shape of the estimated
  effect of \texttt{tfi}:
<<>>=
plot( mx )
@ %
...
\item However, \texttt{plot.gam} does not give you the \emph{absolute}
  ...
  covariates, by setting up a prediction data frame. Note that age in
  the model specification is entered as \texttt{doe-dob}, hence the
  prediction data frame must have these two variables and not the age,
  but it is only the difference that matters for the prediction:
<<pred,fig=TRUE>>=
...
\item Apart from the baseline timescale, time since NRA, the time
  ...
  --- this can be read of the plot with the boxes. With this rather
  limited number of events we can certainly not expect to be able to
  model anything more complicated than a linear trend with time since
  remission.  The variable we want to have in the model is current
  date (\texttt{per}) minus date of remission (\texttt{dor}):
  \texttt{per-dor}, but \emph{only} positive values of it. This can be
  fixed by using \texttt{pmax()}, but we must also deal with all those
  who have missing values of \texttt{dor}, so construct a variable
  which is 0 for persons in \texttt{NRA} and time since remission for
  persons in \texttt{Rem}:
<<>>=
sLc <- transform( sLc, tfr = pmax( (per-dor)/10, 0, na.rm=TRUE ) )
@ %

\item Now expand the model with this variable:
<<time-since-rem>>=
mPx <- gam( cbind(lex.Xst %in% EP,lex.dur) ~ s( tfi, k=10 ) +
            factor(sex) + I((doe-dob-40)/10) + I(lex.Cst=="Rem") + tfr,
            family = poisreg, 
              data = sLc )
round( ci.exp( mPx ), 3 )
@ %
What can you say about the effect of time since remisssion on the rate
of ESRD?
\end{enumerate}
...
\item The rates of ESRD were modelled by a Poisson model with
  ...
  that only the records that relate to the \texttt{NRA} state can be
  used:
<<rem-inc-mgcv>>=
...
ci.exp( mr, pval=TRUE )
@ %
However it would be more convenient to use \texttt{gam.Lexis} with the
\texttt{to} argument, whic will model all transitions \texttt{into}
the states named in the argument \texttt{to} (in this case only one): 
<<>>=
...
\item Suppose we want predictions for men aged 50 at
  ...
  \texttt{lex.dur} and \texttt{lex.Xst} will be ignored --- these two
  variables are the response variables to be simulated).
 
  Note that in order to carry over the necessary attributes from the
  \texttt{Lexis} object \texttt{sLc} to our initiation data set we
  construct the initiation data as a one-line subset of \texttt{sLc},
  and just overwrite all values:
<<>>=
inL <- sLc[1,1:11]
timeSince(inL)
inL[1,"lex.id"] <- 1
...
\item The other input for the simulation is the transitions, which is
  ...
  the list will be model objects (\texttt{glm}, \texttt{gam} or
  \texttt{coxph}), in this case the models we just fitted, describing
...
\item Now count how many persons are present in each state
  at each time for the first 10 years after entry (at age 50). This
  can be done by using \texttt{nState}. Try:
<<nState>>=
nStm <- nState( subset(sM,sex=="M"), 
                at=seq(0,10,0.1), from=50, time.scale="age" )
nStf <- nState( subset(sM,sex=="F"), 
                at=seq(0,10,0.1), from=50, time.scale="age" )
head( nStf )
@ %
What is in the object \texttt{nStf}?

\item With the counts of persons in each state at the designated time
  points (in \texttt{nStm}), compute the cumulative fraction over the
  states, arranged in order given by the permutation \texttt{pm}. W
  just check that the states apper in the right order with the
  permuation \texttt{prm}:
<<pState>>=
prm <- c(2,1,3,4)
colnames( nStm )[prm]
...
plot( ppf )
@ %
What does the plot show? Is this useful?

\item Now try to improve the plot so that it is easier to read, and
  easier to compare men and women. Before you run each line, read it
  carefully and explain what the command does. We could do:
<<new-nState,fig=TRUE>>=
par( mfrow=c(1,2), las=1 )
plot( ppm, col=c("limegreen","red","#991111","forestgreen") )
lines( as.numeric(rownames(ppm)), ppm[,"Rem"], lwd=4 )
text( 59.5, 0.75, "Men", adj=1, col="white", font=2, cex=1.2 )
...
text( 59.5, 0.75, "Women", adj=0, col="white", font=2, cex=1.2 )
axis( side=2, at=0:10/10, label=NA )
