...
\SweaveOpts{results=hide, prefix.string=./graph/tidyverse}

\section{Data manipulation with \textit{tidyverse}}

\subsection{Introduction}

In this chapter we will reproduce \textit{more or less} the same outputs as in chapter 1.2 and 1.4 using 
\texttt{tidyverse} packages suit.

The main goal is to familiarize you with some of the main \texttt{tidyverse} features.
This is an optional practical for participants that have already good basic R basis. 
All the rest of the course can be done without knowing \texttt{tidyverse}.
...
\textbf{Note:} the \texttt{by} argument indicates which column should be use to make the join. In some 
...
We will go more into the details on how to produce graphs with \texttt{ggplot2} in another practical.
Just a quick example of graphic using \texttt{ggplot2}.
...
As you can see, plots from \texttt{ggplot} family are built sequentially using the \texttt{+} operator for each additional element.

\subsection{pivoting data with tidyr}

\texttt{dplyr} often comes with \texttt{tidyr} when we are performing data manipulation. \texttt{tidyr}
main features is to reshape tables from long to wide format and vis versa. Let's have an example.

Let's for instance transform in wide format the previously created \texttt{birth\_per\_ageg} table.
...
This table can easily be format back in long format:
...
Another package from \texttt{tidyverse} that can be introduced here is \texttt{readr} that contains
an equivalent function of all the core \texttt{R} data.frame reading functions (e.g. \texttt{read.table()}, \texttt{read.csv()}, \texttt{read.delim()}, ...). The main change is that data are loaded in \texttt{R} as \texttt{tibble} instead of \texttt{data.frame}, type of variables (columns) are guessed if possible, and some extra data checking tests are performed.

Let's compare this differences with \texttt{fem} dataset.
...
\textbf{note:} in case you do not understand the last lines and the \texttt{map()} call, it will be explained in the next section on \texttt{purrr} pacakge. 
...
As you can see in addition to inferring type of columns in the input data (here some dates), using
\texttt{readr} to load you dataset can help you to detect inconsistencies in input data formatting even if there are no true problem here.

If you are interested, you explore the other functions of \texttt{readr} and see how you can tune it.


\subsection{String manipulation with stringr}

Another popular \texttt{tidyverse} popular package is \texttt{stringr} package. This package is 
specialized in the string manipulation. Here are couple of examples.
...
As you can see, the output of \texttt{str\_locate\_all} is a list (one element per character string) containing
a 2 column table with one line for each match. The first column (start) being the position of the beginning
of the match and the second one (end) being the end of the match. In our case, since we are searching for
a single character match, this 2 indexes are always the same.
...
\subsection{purrrr package to apply functions to list}
...
The simplest example could be to apply the same function to a list of elements.
As an example let's add "country: " in front of all our 

Apply a function to each element of the vector using map(). Here producing the mean of some grades per 
class:
...
By default \texttt{map()} return a list. One of the nice feature of \texttt{purrr} functions is to be
able to specify the type of output you want (e.g. \texttt{\_dbl} for numeric, \texttt{\_chr} for characters, ...).
...
\texttt{purrr} package has also a set of functions that can be used to apply iteratively a function using
\texttt{reduce} and/or \texttt{accumulate}. The 2 function behave he same way, it takes the 2 first element of
a list, apply a function. The results of the first operation is combine with the third element of the list in the 
same function and so on.. The only difference is that accumulate return intermediate results while reduce return only the final results.
