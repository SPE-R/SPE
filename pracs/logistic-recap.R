...
hist(mel$nvsmall,col="black",breaks=0:50)
hist(mel$nvlarge,col="black",breaks=0:50)
...
round(ci.exp(mnl), 2)
round(ci.exp(mnl,subset="nvl"), 2)


###################################################
### code chunk number 24: logistic-s.rnw:293-296
###################################################
mnls <- glm(cc ~ nvsma4 + nvlar3 + age.cat + sex, family="binomial",
            data=mel)
round( ci.exp( mnls, subset="nv" ), 3 )
...
with( mel, table(fscore,freckles) )
...
round(ci.exp(m1,subset="f"), 2)
m2 <- glm(cc ~ fscore + age.cat + sex, family="binomial",data=mel)
round(ci.exp(m2,subset="f"), 2)
