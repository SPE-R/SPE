<<<<<<< HEAD
\SweaveOpts{results=hide, prefix.string=./graph/cont-eff}
...
>>>>>>> a92faa1bb94bb66aa133c173a0e17cd5bf69e54c
<<data-births, echo=T, fig=F>>=
library(Epi)
data(births)
...
\item A graphical presentation of the fitted curve together with the
...
\item To illustrate arbitrariness and associated problems with
  ...
  results
>>>>>>> a92faa1bb94bb66aa133c173a0e17cd5bf69e54c
<<bweigth-gestwks-Ns10, echo = F, fig=F>>=		 
...
\begin{enumerate}[resume]
\item You must first install R package \texttt{mgcv} into your
  computer.
...
<<bweight-gestwks-mPs-plot, fig=F>>=	
...
\item As in previous models we shall plot the fitted curve together
  ...
  prediction intervals for individual responses. Obtaining these
  quantities from the fitted \texttt{gam} object requires a bit more
...
tdk$Age <- cut(tdk$A, br = 5*(3:16), include.lowest=T, right=F)
   nAge <- length(levels(tdk$Age))
tdk$Per <- cut(tdk$P, br = seq(1943,1998,by=5), include.lowest=T, right=F)
   nPer <- length(levels(tdk$Per))
@ %
>>>>>>> a92faa1bb94bb66aa133c173a0e17cd5bf69e54c
\end{enumerate}
...
non-linear, but it is less clear whether the true period effect
...
On the rate scale the baseline level (per 100000 y) is obtained by
\texttt{exp(1.7096)}
...
of reference value for the period effect, as it was in the categorical
...
\item Graphical presentation of the effects can be improved from that
  supported by \texttt{plot.gam()}. We can, for instance, present the
  age curve to describe the ``mean'' incidence rates by age, averaged
  over the 54 years.  For that purpose we need to merge the intercept
  with the age effect.  The period curve will be expressed in terms of
  rate ratios in relation to the fitted baseline rate, as determined
  by the model intercept.

  In order to produce these plots one needs to extract certain items
  from the fitted \texttt{gam} object \texttt{mPen2} and do some
  calculations. A source script named ``\texttt{plotPenSplines.R}''
  that does all of that can be found from the \texttt{/R} subdirectory
  on the course website.
<<mPen2-plotAge, fig = F>>=
source("http://bendixcarstensen.com/SPE/R/plotPenSplines.R")
lls()
str( model.matrix(mPen2) )
@ 
\end{enumerate}
...
\item Instead of penalized splines one may use ordinary regression
  splines, in which the user specifies the model matrix (basis) by
  choosing a suitable number of knots and their locations. We prefer
  natural splines for the purpose.  Fitting these models can be done
  with \texttt{glm()} such that in the model formula the spline
  components are defined by function \texttt{Ns()}
<<mNs>>=
mNs <- glm( D ~ Ns(A, knots = seq(  15,  75, 10)) +
...
summary( mNs)
@ %
Again, the summary provides not so interpretable quantities, so we
need a graphical representation of the effects.

\item As in previous models we want to describe the age effect on the
  original rate scale merging the intercept. In the categorical model
  we chose the years 1968--72 as the reference period.  Here we assign
  year 1970 as the reference calendar year, so that the age curve
  describes the estimated age effect and the period curve describes
  the rate ratio of each calendar year versus the reference year.  To
  this end we can utilize function \texttt{ci.exp()} with suitable
  contrast matrices for age and period, that comply with the structure
  of the fitted splines for both effects as well as the desire to keep
  year 1970 as the reference calendar year.  All of that can be done
  with the following piece of script
>>>>>>> a92faa1bb94bb66aa133c173a0e17cd5bf69e54c
<<mNs10-plot, fig = F>>=
aa <- 15:79
pp <- 43:96
...
=======
@ %
In \texttt{Epi} 2.26 this can be substantially simplified; note that for
the \emph{rates} we need a full specification of \emph{all} variables
in the model in the \texttt{newdata} argument to \texttt{ci.pred};
whereas the RR relative to a reference point only need specification
of the values of \texttt{P} for which we need the RR (and the
reference). Moreover, the code to extract the effects will be the same
for a different choice of knots or type of spline: 
<<>>=
aa <- 15:79
pp <- 1943:1996
pr <- 1970
par(mfrow=c(1,2))
# rates as of 1970
matplot( aa, ci.pred( mNs, newdata=data.frame(A=aa,P=pr,Y=10^5) ),
          log="y", xlab="Age", ylab="Incidence rate (per 100,000 y)",
          type="l", lty=1, lwd=c(3,1,1), col = c("red", "blue",  "blue") )
# RR relative to 1970
matplot( pp, ci.exp( mNs, ctr.mat=list(data.frame(P=pp),
                                       data.frame(P=pr)), xvar=c("A","Y") ),
          log="y", xlab="Year", ylab="Rate ratio",
          type="l", lty=1, lwd=c(3,1,1), col= c("red", "blue",  "blue") )
@ %
Inspect the curves. Are they very different from those you got
...
\item In order to answer this precisely, we try the same thing with
  the penalized splines (will not work for \texttt{gam} till Epi 2.27,
  though --- here we load the new code directly)
<<>>=
par(mfrow=c(1,2))
matplot( aa, ci.pred( mPen, newdata=data.frame(A=aa,P=1970,Y=10^5) ),
          log="y", xlab="Age", ylab="Incidence rate (per 100,000 y)",
          type="l", lty=1, lwd=c(3,1,1), col = c("red", "blue",  "blue") )
matplot( pp, ci.exp( mPen, ctr.mat=list(data.frame(P=pp),data.frame(P=1970)) ),
          log="y", xlab="Year", ylab="Rate ratio",
          type="l", lty=1, lwd=c(3,1,1), col= c("red", "blue",  "blue") )
@ %
We can put in the estimates from the \texttt{Ns}-based model (broken
lines) to illustrate the difference:
<<PenNs-comp,fig=TRUE,width=10>>=
par(mfrow=c(1,2))
matplot( aa, ci.pred( mPen, newdata=data.frame(A=aa,P=1970,Y=10^5) ),
          log="y", xlab="Age", ylab="Incidence rate (per 100,000 y)",
          type="l", lty=1, lwd=c(3,1,1), col = c("red", "blue",  "blue") )
matlines( aa, ci.pred( mNs, newdata=data.frame(A=aa,P=1970,Y=10^5) ),
          type="l", lty=c("11","33","33"), lend="butt", lwd=c(3,1,1), col = c("red", "blue",  "blue") )
matplot( pp, ci.exp( mPen, ctr.mat=list(data.frame(P=pp),data.frame(P=1970)) ),
          log="y", xlab="Year", ylab="Rate ratio",
          type="l", lty=1, lwd=c(3,1,1), col= c("red", "blue",  "blue") )
matlines( pp, ci.exp( mNs, ctr.mat=list(data.frame(P=pp),data.frame(P=1970)) ),
          type="l", lty=c("11","33","33"), lwd=c(3,1,1), col= c("red", "blue",  "blue") )
abline( v=1970, h=1, lty=3, col="gray")
@ %
One obvious difference is found in the way the error margins for the
period effect are displayed. In the current model a specific reference
calendar year, 1970, was chosen as a fixed reference point, and the
rate ratio for all other calendar times describes a contrast of that
time point versus year 1970. Therefore, there is no uncertainty in the
rate ratio for 1970. In the penalized splines model no time point was
explicitly chosen as a fixed reference, and the fitted period curve as
well as its confidence band aims at estimating the period effect in
relation to the baseline rate, and the uncertainty concerns the
location of the whole curve.

\item Fit another spline model but now with greater numbers of
  knots. For instance, for age you may assign 13 knots from 15 to 75
  years in steps of 5 years, and for period 11 knots from 45 to 95
  years.  Plot also the estimated age and period effects and comment
  the difference to our first spline model.
