...
It is said that Mrs Beeton, the 19th century cook and writer, began
her recipe for rabbit stew with the instruction ``First catch your
rabbit''.  Sadly, the story is untrue, but it does contain an
important moral. \textsf{R} is a language and environment for data
analysis. If you want to do something interesting with it, you need
data.
...
You will need the following files in the sub-directory \textsf{data}
of your working directory: {\tt fem.dat}, {\tt fem-dot.dat}, {\tt
  fem.csv}, {\tt fem.dta} (Reminder: use \texttt{setwd()} to set your
working directory).
...
To understand why there are two functions, inspect the contents of
the function \verb+read.csv()+ by typing its name
<<results=verbatim>>=
read.csv
@ 

The first two lines show the arguments to the \verb+read.csv()+
function and their default values (\verb+header=TRUE+, etc) The next
two lines show the {\em body} of the function, which shows that the
default arguments are simply passed verbatim onto the
\verb+read.table()+ function. Hence \verb+read.csv()+ is a {\em
wrapper} function that chooses the correct arguments for
\verb+read.table()+ for you.  You only need to supply the name of the
CSV file and all the other details are taken care of.

Now inspect the \verb+read.csv2+ function to find the difference
between this function and \verb+read.csv+.

\begin{quote}
  {\bf Explanation:} The CSV format is not a single standard.
  The file format depends on the {\em locale} of your computer -- the
  ...
  semi-colon ``;''. The \verb+read.csv()+ function is used for the first
  format and the \verb+read.csv2()+ function is used for the second
  format.
\end{quote}
...
On Microsoft Windows, you can copy values directly from an open
Excel spreadsheet using the clipboard. Highlight the cells you
want to copy in the spread sheet and select copy from the pull-down
edit menu. Then type 
\verb+read.table(file="clipboard")+ to read
the data in.  Beware, however, that the clipboard on Windows operates
on the WYSIWYG principle (what-you-see-is-what-you-get). If you have
a value $1.23456789$ in your spreadsheet, but have formatted the cell
so it is displayed to two decimal places, then the value read into
