...
\SweaveOpts{eval=TRUE,keep.source=TRUE,eps=FALSE,include=TRUE}
...
<<survcdf1, echo=T, fig=T, height=6, width=7>>=
plot(s.all)
...
<<cdfstage, echo=T, fig=T, height=6, width=7>>=
s.stg  <- survfit(suob ~ stage, data= orca)
col5 <- c("green", "blue", "black", "red", "gray")
plot(s.stg, col= col5, fun="event", mark.time=F, lwd=3 )
s.stg
@
...
<<chstage, echo=T, fig=T, height=6, width=12>>=
par(mfrow=c(1,2))
...
\item In the lecture handouts %(p. 34, 37) 
...
stat.table( list( sex, agegr), 
            list( count(), percent(agegr) ),
            margins = TRUE, data = orca )
@
Male patients are clearly younger than females in this data set.

Now, plot Kaplan--Meier curves jointly classified by sex and age.
<<cdfsexage, echo=T, fig=T, height=6, width=7>>=
s.agrx <- survfit(suob ~ agegr + sex, data=orca)
par(mfrow=c(1,1))
plot(s.agrx, fun="event", mark.time=F, xlim = c(0,15),
             col=rep(c("red","blue"),3), 
             lty=rep(c(2,1,5),each=2), lwd=3 )
@ %
...
\begin{enumerate}[resume]
\item Use the \texttt{survfit}-function in \texttt{survival} package
  with option \texttt{type="mstate"}.
<<cif1a, echo=T>>=
library(survival)
...
<<plotcif2,echo=T,fig=T,height=6,width=10>>=
col5 <- c("green", "blue", "black", "red", "gray")
...
\texttt{crr()}. You also need to attach the data frame, because the
the \texttt{crr} function does not have a \texttt{data=} argument.
<<fg1, echo=T>>=
library(cmprsk)
attach(orca2)
