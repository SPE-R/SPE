...
\section{Survival analysis with competing risks: Oral cancer patients}
...
\item In the lecture handouts it was observed that
...
\subsection{Modelling event-specific hazards}
...
\item Draw a box diagram of the two-state set-up of competing transitions. Run first th e following command line
\begin{Schunk}
\begin{Sinput}
boxes( orca.lex )
\end{Sinput}
...
\subsection{Optional: Poisson regression as an alternative to Cox model}
...
\item First we form the necessary {\tt lexis} object by just taking
...
\item We are ready to fit the desired Poisson model for oral cancer death
...
\item The estimated baseline looks quite ragged when based on 71 separate
