\SweaveOpts{results=hide,keep.source=TRUE,include=FALSE,eps=FALSE} %  ,prefix.string=./graph/rates}
...
\item Suppose you have $15$ events during $5532$ person-years. Let's
  ...
  person-years; therefore 5.532) and the first version of an approximate confidence
  interval:
<<>>=
library( Epi )
...
You are able to estimate $\lambda$ and compute its CI with a Poisson model,
...
\item The summary method produces too much output. You can extract CIs
...
rates
@ %
\item Fit the same model as before, assuming a single rate to the data
  for the separate periods. Compare the result from previous ones
<<>>=
...
@
and compare the two models using \texttt{anova()} with the argument
\texttt{test="Chisq"}:
<<>>=
...
$\log(\widehat\lambda_1) - \log(\widehat\lambda_0)$ is estimated as
\begin{eqnarray*}
   {\mbox{var}}(\log(\RR)) 
...
\item Verify that this is the confidence interval you get when you fit
  an additive model with exposure as factor:
<<>>=
