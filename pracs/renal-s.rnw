...
\item We use the \texttt{Lexis} function to declare the data as survival
  data with age, calendar time and time since entry into the study as
  timescales. Note that any coding of event $>0$ will be labeled
  ``ESRD'', i.e. renal death (death of kidney (transplant or
  dialysis), or person).
...
\item We can produce a slightly more fancy Lexis diagram. Note that we
  ...
  so that the use \texttt{mai} to specify the margins of the plot
...
<<>>=
pdf( "./graph/renal-Lexis-fancy", height=80/5+1, width=40/5+1 )
...
summary( Lc )
@ %
Note that we have two different ESRD states depending on wheter the
...
The number in the boxes are the person-years and the number of persons
starting (left) and ending (right) their follow-up in each state; the
numbers on the arrows are the number of transitions and the overall
transition rates (in per 100 PY, by the \textrm{\tt scale.R=100}).}
...
\begin{figure}[tb]
  \centering
  \includegraphics[height=0.7\textheight,keepaspectratio]{./graph/renal-Lexis-rem}
  \caption{\it Lexis diagram for the split data, where time after
    remission is shown in green.}
  \label{fig:renal-Lexis-rem}
\end{figure}
}
...
ci.exp( mp )
@ %
We see that the effecst are pretty much the same as from the
Cox-model.
...
\insfig{pred}{0.7}{Rates of ESRD by time since NRA for a man aged 50 at
  start of NRA. The gray line is the curve fitted by \texttt{gam}, the
...
rate 2.6 times higher than for men, bote when using \texttt{glm} and
...
head( nStf )
@ %
  We see that we get a count of perosn in each state at timepoints
...
\insfig{plot-pp}{0.7}{The default plot for a \texttt{pState} object,
  bottom to top: Alive no remissiom, alive remission, dead remission,
  dead no remission.}
...
@ %
\insfig{new-pState}{0.9}{Predicted state occupancy for men and women
...
perons with remission are alive after 5 years. 
