\SweaveOpts{results=hide,keep.source=TRUE,include=FALSE,eps=FALSE,prefix.string=./graph/causInf2}
...
\end{align*}
<<true contrasts>>=
dd <- transform(dd, EY1.ind = EY(x=1, z1, z2, z3, z4),
...
dd$yp1 <- predict(mY, newdata=data.frame( x=rep(1,N),       # x=1
          dd[,c("z1","z2","z3","z4")]), type = "response")
dd$yp0 <- predict(mY, newdata=data.frame( x=rep(0,N),       # x=0
          dd[,c("z1","z2","z3","z4")]), type = "response")
@
\item
...
See \href{https://doi.org/10.1093/aje/kww165}{\color{blue}Schuler and Rose (2017)}
for more details
