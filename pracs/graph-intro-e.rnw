\SweaveOpts{results=hide, debug=FALSE, prefix.string=./graph/graph-intro}

\section{Graphics in R}

There are three kinds of plotting functions in R:
\begin{enumerate}
\item Functions that generate a new plot, e.g. \texttt{hist()} and
  \texttt{plot()}.
\item Functions that add extra things to an existing plot,
  e.g. \texttt{lines()} and \texttt{text()}.
\item Functions that allow you to interact with the plot, e.g.
...
\subsection{Simple plot on the screen}
...
\begin{enumerate}[resume]
  \item Make a plot of the birth weight versus maternal age with
<<>>=
with(births, plot(matage, bweight) )
@
\item Label the axes with
<<>>=
...
\texttt{colours()} or, equivalently, \texttt{colors()} (\R\ allows you to
...
@
Note: when the number of data points on a scatter plot is large, you may also want to decrease the point size: to get points that are 50\% of the original size, add the parameter \texttt{cex=0.5} (or another number <1 for different sizes). 

\subsection{Adding to a plot}
...
Suppose we wish to recreate the plot of birthweight {\em vs} gestational
weeks using different colours for male and female babies. To start with
an empty plot, with \texttt{type='n'} argument.
...
\subsubsection{Using indexing for plot elements}
...
@
Note we add 1 because \verb+( matage >= 40 )+ generates a logic
...
%\begin{enumerate}[resume]
%\item Make a three level factor for maternal age with cutpoints at
...
%\item Use this to make the plot of bweight versus gestational weeks with three
%  different plotting symbols. (Hint: Indexing with a factor
%automatically gives indexes 1,2,3 etc.).
%\end{enumerate}

\subsubsection{Generating colours}
...
\subsection{Saving your graphs for use in other documents}
...
\framebox{Help}$\rightarrow$ \framebox{Html help}$\rightarrow$
\framebox{Packages}$\rightarrow$ \framebox{graphics}$\rightarrow$
\framebox{P}$\rightarrow$ \framebox{par}.
...
in the current plot. \hspace*{16em}
...
\subsection{Interacting with a plot}
...
\texttt{births}. If there is no point nearby, \R\ will print a warning
...
\begin{enumerate}
\item Use \texttt{identify()} to find which records correspond to the
