...
\subsection{Stratified effects and interaction or effect modification} %% 1.8.5
...
\subsection{Third degree polynomial of {\tt gestwks} }  %% 1.8.9
...
<<bweight-by-gestwks-cubic-pred, fig=F>>=
...
for this purpose. The \texttt{mgcv} package is a ``recommended'' package
that is included in every distribution of R so you do not need to download
it from CRAN.
...
<<bweight-gestwks-mPen-plot, fig = F>>=	
...
\subsection{Extra (if you have time): Frequency data}  %% 1.8.10
