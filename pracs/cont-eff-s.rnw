...
<<data-births, echo=TRUE, fig=TRUE>>=
library(Epi)
data(births)
par(mfrow=c(1,1))
...
@
\iten Now, create a vector of $x$-values and compute the fitted/predicted values as well
...
\subsection{Extra: Generalized additive model with natural regression splines}
