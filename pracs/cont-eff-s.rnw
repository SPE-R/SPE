\SweaveOpts{results=verbatim, prefix.string=./graph/cont-eff}
...
<<bweigth-gestwks-Ns10, echo=TRUE, fig=TRUE>>=		 
...
<<bweight-gestwks-mPs-plot, fig=TRUE>>=	
...
for (p in 1:nPer)
   lines( seq(17.5, 77.5, by = 5), tab[3, 1:nAge, p], type = 'o', pch = 16 ,
	   lty = rep(1:6, 2)[p] )
@ %
There is also a function \texttt{rateplot} that does default plotting of
rate tables (see the help page for \texttt{rateplot}):
<<rateplot,fig=TRUE>>=
par( mfrow=c(1,1) )
...
abline(h = 0, lty=3)
@ %
The dashed lines describe the 95 \% confidence band for the
pertinent curve.  One could get the impression that year 1968 would be
some kind of reference value for the period effect, as it was in the
categorical model previously fitted. This is not the case, however,
because {\tt gam()} by default parametrizes the spline effects such
that the reference level, at which the spline effect is nominally
zero, is the overall ``grand mean'' value of the log-rate in the
data. This corresponds to the principle of {\it sum contrasts} ({\tt
  contr.sum}) for categorical explanatory factors.
...
Here we assign year 1970 as the reference date,
so that the age curve describes the estimated age effect
and the period curve describes the rate ratio versus the reference date. 
To this end we can utilize functions \texttt{ci.pred} to make
predicted rates fo the reference date, and \texttt{ci.exp}
to extract predicted rate-ratios relative to the reference (see the
help page for \texttt{ci.exp}).

What is needed are data frames with values for the covariates
\texttt{A}, \texttt{P} and \texttt{Y}, one data frame for the
predictions and two data frames representing the rate ratios we want:
<<predRR>>=
summary( mNs )
aa <- 15:79
pp <- 1943:1996
...
The nice thing about the approach is that exactly the same piece of
code works for the \texttt{gam} model, so we can show the \texttt{gam}
model with shades on top of the \texttt{Ns} resuts:
<<gamNs,fig=TRUE>>=
par(mfrow=c(1,2))
